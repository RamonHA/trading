...
class Asset(TimeSeries):
    ...
    def df_gbm_api(self):
        ...
        # if self.intervalo == "w":
        #     df = yf.download(self.symbol, start = inicio, end = fin, interval='1d' ,progress=False)
        # else:
        if (self.intervalo != "min"):
            df = yf.download(self.symbol, start = self.inicio, end = self.fin, interval= aux[self.intervalo] ,progress=False)
        else:
            inicio = datetime.combine(self.inicio, datetime.min.time())
            fin = datetime.combine(self.fin, datetime.min.time())

            if (fin - inicio).days <= 7:
                df = yf.download(self.symbol, start = inicio, end = fin, interval= aux[self.intervalo] ,progress=False)
            else:    
                ...
                while True:
                    ...
                    try:
                        df_aux = yf.download(self.symbol, start = inicio_aux, end = fin_aux, interval= aux[self.intervalo] ,progress=False)
                    except:
                        df_aux = pd.DataFrame()
    ...
    def df_gbm_archivo(self):
        ...
        try:
            df = pd.read_csv(pwd.format( self.broker, aux[ self.intervalo ], self.symbol.replace(".", "") ))
        except:
            return None
